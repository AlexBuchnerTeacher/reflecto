...
import '../services/habit_migration.dart';
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          if (_isAdmin(uid))
            IconButton(
              tooltip: 'Kategorien migrieren',
              icon: const Icon(Icons.sync_alt),
              onPressed: () async {
                if (uid == null) return;
                final migration = HabitMigration();
                final count = await migration.migrateUserHabits(uid);
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('$count Habit(s) migriert')),
                  );
                }
              },
            ),
