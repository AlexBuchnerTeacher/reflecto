...
final weekEntriesProvider = FutureProvider.family<List<JournalEntry>, DateTime>((ref, anyDayInWeek) async {
  final user = FirebaseAuth.instance.currentUser;
...
final todayDocProvider = StreamProvider<DocumentSnapshot<Map<String, dynamic>>>((ref) {
  final user = FirebaseAuth.instance.currentUser;
  if (user == null) return const Stream.empty();
...
final weeklyReflectionProvider = StreamProvider.family<Map<String, dynamic>?, String>((ref, weekId) {
  final user = FirebaseAuth.instance.currentUser;
  if (user == null) return const Stream.empty();
  final refDoc = FirebaseFirestore.instance.doc('users/${user.uid}/weekly_reflections/$weekId');
  return refDoc.snapshots().map((s) => s.data());
});

// Family providers by date for day views
final dayEntryProvider = StreamProvider.family<JournalEntry?, DateTime>((ref, date) {
  final user = FirebaseAuth.instance.currentUser;
  if (user == null) return const Stream.empty();
  final svc = ref.read(_svcProvider);
...
final dayDocProvider = StreamProvider.family<DocumentSnapshot<Map<String, dynamic>>, DateTime>((ref, date) {
  final user = FirebaseAuth.instance.currentUser;
  if (user == null) return const Stream.empty();
  String two(int n) => n.toString().padLeft(2, '0');
...
typedef UpdateDayField = Future<void> Function(String uid, DateTime date, String field, dynamic value);

final updateDayFieldProvider = Provider<UpdateDayField>((ref) {
  final svc = ref.read(_svcProvider);
  return (String uid, DateTime date, String field, dynamic value) => svc.updateField(uid, date, field, value);
});
